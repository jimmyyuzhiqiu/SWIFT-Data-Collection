...
def pick_from_blocks(picker, *blocks: list[str]) -> str:
    """
    Try multiple block line lists in order, return first non-empty pick.
    Used for parallel 50K/50F logic.
    """
    for b in blocks:
        v = picker(b)
        if v:
...
def extract_step3_record(text: str) -> dict:
    ...
    b50f = extract_block_lines(text, "50F")
    b59  = extract_block_lines(text, "59")
    b59f = extract_block_lines(text, "59F")
    b59k = extract_block_lines(text, "59K")
    b52a = extract_block_lines(text, "52A")
    b57a = extract_block_lines(text, "57A")

    if direction == "OUT":
        # 50K / 50F parallel logic
        client_acct = pick_from_blocks(pick_account_line1, b50k, b50f)
        ...
        # USD IN: 59F / 59K parallel logic; EUR IN: 59 only
        if ccy == "USD":
            client_acct = pick_from_blocks(pick_account_line1, b59f, b59k)
        else:
            client_acct = pick_account_line1(b59)
        # 50K / 50F parallel logic
        cp_name     = pick_from_blocks(pick_name_line2, b50k, b50f)
        cp_acct     = pick_from_blocks(pick_account_line1, b50k, b50f)
